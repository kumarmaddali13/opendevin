from agenthub.codeact_planning_agent.action_parser import CodeActPlanningResponseParser
from agenthub.codeact_planning_agent.prompt import (
    COMMAND_DOCS,
    EXAMPLES,
    GITHUB_MESSAGE,
    SYSTEM_PREFIX,
...
def action_to_str(action: Action) -> str:
    ...
    elif isinstance(action, SavePlanAction):
        plan_str = '\n'.join(action.plan)
        return f'{action.thought}\n<save_plan>\n{plan_str}\n</save_plan>'
    elif isinstance(action, PlanStepAction):
        return f'{action.thought}\n<save_plan>\n{action.step}\n</save_plan>'
...
class CodeActPlanningAgent(Agent):
    VERSION = '1.0'
    """
    ...
    *Example of CodeActAgent with `gpt-4-turbo-2024-04-09` performing a data science task (linear regression)*
    ...
    action_parser = CodeActPlanningResponseParser()
    ...
    def step(self, state: State) -> Action:
        ...
        - SavePlanAction(plan) - save the plan
        - PlanStepAction(plan) - move to a step in the plan
        ...
        if latest_user_message:
            ...
            current_plan = state.get_current_plan_str()
            if current_plan:
                latest_user_message['content'] += (
                    f'\n\nCURRENT_PLAN (**current step**):\n{current_plan}'
                )
            latest_user_message['content'] += (
